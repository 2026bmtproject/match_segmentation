...
抓出候選片段後，對每段取 2 個代表幀，並與其他候選片段的代表幀做跨片段比對，
最後再依據 Cross_Diff_Avg 做二次篩選，輸出成 CSV。
...
規則：
    - 低於分界的幀視為候選片段
    - 兩個候選片段的間隔小於 2 frame 時，合併為同一片段
    - 合併後僅保留長度至少 0.5 秒的片段
    - 每個候選片段平分成兩份，取每份的中間 frame
    - 每個候選片段的 2 個代表 frame，會和其他候選片段的 2 個代表 frame 進行跨片段比對
    - 所有 MAD 分數會四捨五入到整數位
    - 依據 rounded min(Cross_Diff_Avg) 設門檻：
        threshold = round(min_avg * (1 + pct))
      並保留 Cross_Diff_Avg <= threshold 的片段

用法:
    python bre6_c2_3.py input.mp4 output.csv
    python bre6_c2_3.py test3.mp4 test3_c2_3_segments.csv --diff-csv test3_bre6.csv

可調參數:
    --avg-thr-scale: 0~1，控制百分比強度（越小越嚴格）
        - 0   : 不放寬（僅保留最小值）
        - 1   : 使用動態百分比的上限
        - 預設 0.7
    --avg-thr-pct: 直接指定百分比（0~0.2），指定後會覆蓋動態百分比

若未提供參數:
    - input.mp4 會使用目前目錄第一個 .mp4
    - output.csv 會命名為 <影片檔名>_c2_3_segments.csv
...
def suggest_avg_threshold_pct(segment_count: int) -> float:
    """依候選片段數建議 Cross_Diff_Avg 放寬百分比（上限 10%）。"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="FrameDiff(MAD) + GMM 分段 + 跨片段比對(每段2幀) + Cross_Diff_Avg 篩選",
    )
    parser.add_argument(
        "video_path",
        nargs="?",
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="輸出 CSV 路徑（預設 <影片名>_c2_3_segments.csv）",
    )
    parser.add_argument(
        "--diff-csv",
        dest="diff_csv",
    ...
    parser.add_argument(
        ...
        help="Cross_Diff_Avg 門檻百分比強度(0~1，越小越嚴格，預設 0.7)",
    )
    parser.add_argument(
        "--avg-thr-pct",
        dest="avg_thr_pct",
        type=float,
        default=None,
        help="直接指定門檻百分比(例如 0.05 代表 +5%%，會覆蓋動態比例)",
    )
...
def main() -> None:
    ...
    output_csv = (
        args.output_csv
        if args.output_csv
        else f"{os.path.splitext(os.path.basename(video_path))[0]}_c2_3_segments.csv"
    )

    print("=" * 60)
    print("bre6_c2_3: FrameDiff(MAD) + GMM + 候選片段跨片段比對(每段2幀) + 最終篩選")
    print("=" * 60)
