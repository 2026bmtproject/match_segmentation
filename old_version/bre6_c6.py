...
用法:
    python bre6_c6.py test2.mp4 test2_c6_segments.csv

若未提供參數:
    - input.mp4 會使用目前目錄第一個 .mp4
    - output.csv 會命名為 <影片檔名>_c6_segments.csv
"""
...
def filter_segments_by_gmm_cluster(
    segments: list[tuple[int, int]],
    cross_avgs: list[float],
    keep_ratio: float = 0.5,
):
    """
    用 GMM clustering + cluster內一致性 過濾
    """

    if len(cross_avgs) == 0:
        return []

    log_scores = np.log10(np.maximum(np.array(cross_avgs), 1e-12))
    ...
    # 找最小 mean（最像的 cluster）
    rally_cluster = np.argmin(means)

    # 取這個 cluster 的 index
    candidate_indices = [i for i, l in enumerate(labels) if l == rally_cluster]
    ...
    for i in candidate_indices:
        ...
        for j in candidate_indices:
            if i == j:
                continue
            diffs.append(abs(cross_avgs[i] - cross_avgs[j]))
        ...
        scores.append((i, avg_diff))

    # 排序（越小越穩定）
    scores.sort(key=lambda x: x[1])

    keep_n = max(1, int(len(scores) * keep_ratio))

    selected_indices = [i for i, _ in scores[:keep_n]]
...
def main() -> None:
    ...
    filtered_segments = filter_segments_by_gmm_cluster(
      candidate_segments,
      cross_avgs,
      keep_ratio=0.4   # 可以調 0.3 ~ 0.7 小=嚴格
    )
